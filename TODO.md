...
- [ ] Confirm the **changelog** automation stays aligned with
      ...
      changelog flow).
- [x] Repoint the workflow `uses:` refs from `mrdoodles/*` to `lite-actions/*`.
      Done — the actions are published under the org and the references no
      longer depend on GitHub's rename redirect. `mrdoodles/versioning-tests`
      is deliberately unchanged: that repository has not moved.
